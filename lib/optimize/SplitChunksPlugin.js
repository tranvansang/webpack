		...
		compiler.hooks.thisCompilation.tap("SplitChunksPlugin", compilation => {
			...
			compilation.hooks.optimizeChunks.tap(
				...
				chunks => {
					...
					/** @type {Set<string>} */
					const alreadyValidatedParents = new Set();
					/** @type {Set<string>} */
					...
					const addModuleToChunksInfoMap = (
						...
						if (
							!alreadyValidatedParents.has(parentValidationKey) &&
							!alreadyReportedErrors.has(name)
						) {
							alreadyValidatedParents.add(parentValidationKey);
							const existingChunk = compilation.namedChunks.get(name);
							if (existingChunk) {
								// Module can only be moved into the existing chunk if the existing chunk
								// is a parent of all selected chunks
								const referencedChunks = existingChunk.getAllReferencedChunks();
								if (
									!selectedChunks.every(chunk => referencedChunks.has(chunk))
								) {
